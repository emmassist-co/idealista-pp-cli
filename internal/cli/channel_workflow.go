...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  idealista-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  idealista-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"home", "login-and-register-api", "pt", "pt-ajax-listing-controller-ad-contact-info-for-detail-ajax", "pt-home-user-searches", "pt-locations-suggest-sale-home"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  idealista-pp-cli workflow status

  # Show status as JSON
  idealista-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("idealista-pp-cli")
			}
