...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "idealista-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("IDEALISTA_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("IDEALISTA_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "idealista-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/idealista-pp-cli/feedback.jsonl.
When ` + "`IDEALISTA_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`IDEALISTA_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  idealista-pp-cli feedback list
  idealista-pp-cli feedback list --limit 5
  idealista-pp-cli feedback list --json`,
