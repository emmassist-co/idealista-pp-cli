...
func TestWhichCommand_LocationSuggestionsQueryFindsSearchLocations(t *testing.T) {
	var flags rootFlags
	cmd := newRootCmd(&flags)
	var out bytes.Buffer
	cmd.SetOut(&out)
	cmd.SetErr(&out)
	cmd.SetArgs([]string{"--json", "which", "location suggestions"})
	if err := cmd.Execute(); err != nil {
		t.Fatalf("Execute: %v", err)
	}
	var got map[string][]whichMatch
	...
	if got["matches"][0].Entry.Command != "search locations" {
		t.Fatalf("top command = %q, want search locations", got["matches"][0].Entry.Command)
	}
}

func TestWhichCommand_SavedSearchesQueryFindsSearchSaved(t *testing.T) {
	var flags rootFlags
	cmd := newRootCmd(&flags)
	var out bytes.Buffer
	cmd.SetOut(&out)
	cmd.SetErr(&out)
	cmd.SetArgs([]string{"--json", "which", "saved searches"})
	if err := cmd.Execute(); err != nil {
		t.Fatalf("Execute: %v", err)
	}
	var got map[string][]whichMatch
	...
	if got["matches"][0].Entry.Command != "search saved" {
		t.Fatalf("top command = %q, want search saved", got["matches"][0].Entry.Command)
	}
}
