...
func TestMaskCredentialText_MasksCookieHeader(t *testing.T) {
	t.Parallel()

	c := &Client{
		Config: &config.Config{
			Headers: map[string]string{"Cookie": "datadome=super-secret-cookie"},
		},
	}
	got := c.maskCredentialText("Cookie: datadome=super-secret-cookie")
	if strings.Contains(got, "super-secret-cookie") {
		t.Fatalf("cookie leaked in masked text: %q", got)
	}
	if !strings.Contains(got, "****") {
		t.Fatalf("masked text = %q, want masked token marker", got)
	}
}
