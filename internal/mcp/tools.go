...
import (
	...
	"idealista-pp-cli/internal/cli"
	"idealista-pp-cli/internal/client"
	"idealista-pp-cli/internal/config"
	"idealista-pp-cli/internal/mcp/cobratree"
...
func RegisterTools(s *server.MCPServer) {
	...
	s.AddTool(
		mcplib.NewTool("search",
			mcplib.WithDescription("Full-text search across synced local SQLite data only. This does not perform a live public-site search. Requires sync first."),
		...
		if err != nil {
			...
			case strings.Contains(msg, "HTTP 401"):
				return mcplib.NewToolResultError("authentication failed: " + msg +
					...
					"\n      See API docs: https://www.idealista.pt" +
					"\n      Run 'idealista-pp-cli doctor' to check auth status."), nil
...
func newMCPClient() (*client.Client, error) {
	...
	cfgPath := filepath.Join(home, ".config", "idealista-pp-cli", "config.toml")
	cfg, err := config.Load(cfgPath)
	if err != nil {
		return nil, fmt.Errorf("loading config: %w", err)
	}
	c, err := client.New(cfg, 60*time.Second, 2)
	if err != nil {
		return nil, err
	}
...
func dbPath() string {
	home, _ := os.UserHomeDir()
	return filepath.Join(home, ".local", "share", "idealista-pp-cli", "data.db")
}
...
func handleContext(_ context.Context, _ mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
	ctx := map[string]any{
		"api":         "idealista",
		"description": "Curated website-facing CLI spec derived from an authorized Idealista.pt HAR capture.",
		"archetype":   "generic",
		"tool_count":  10,
		// tool_surface tells agents which surface a capability lives on.
		"tool_surface": "MCP exposes typed endpoint tools plus a runtime mirror of user-facing CLI commands. Endpoint tools keep typed schemas; command-mirror tools shell out to the companion idealista-pp-cli binary.",
		"resources": []map[string]any{
			{
				"name":        "addetail-recommendation",
				"description": "Detail recommendation modules",
				"endpoints":   []string{"get_addetail_recommendation"},
				"searchable":  true,
			},
			{
				"name":        "detail",
				"description": "Listing detail metadata",
				"endpoints":   []string{"get_datalayer"},
				"searchable":  true,
			},
			{
				"name":        "home",
				"description": "Home and search-box metadata",
				"endpoints":   []string{"list_operation_typology"},
				"syncable":    true,
			},
			{
				"name":        "login-and-register-api",
				"description": "Session and login-state helpers",
				"endpoints":   []string{"list_od"},
				"syncable":    true,
			},
			{
				"name":        "pt",
				"description": "Portuguese portal endpoints",
				"endpoints":   []string{"get_configuration", "get_openDetailGallery", "list_adContactInfoForDetail.ajax", "list_home", "list_pager", "list_user_searches"},
				"syncable":    true,
				"searchable":  true,
			},
		},
