# Idealista Printed CLI Agent Guide

This directory is a generated `idealista-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
idealista-pp-cli doctor --json
idealista-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
idealista-pp-cli which "<capability>" --json
idealista-pp-cli <command> --help
```
...
```bash
idealista-pp-cli <command> --agent
```
...
```bash
idealista-pp-cli <command> --help
idealista-pp-cli <command> --dry-run --agent
```
